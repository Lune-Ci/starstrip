...
export function AuthBridge() {
  ...
  useEffect(() => {
    if (status === "authenticated" && session?.user) {
      ...
      const provider = ((session.user as any).provider ?? "credentials") as
        | "google"
        | "apple"
        | "facebook"
        | "credentials";
      if (id) {
        // If user is already correctly set, skip to avoid loops/redundant updates
        ...
        if (guestId) {
          ...
          const fromFav = fav.profiles[guestId] ||
            fav.profiles.global || { attractions: [], restaurants: [] };
          const toFav = fav.profiles[id] || {
            attractions: [],
            restaurants: [],
          };
          ...
          const mergedFav = {
            attractions: dedupById([
              ...toFav.attractions,
              ...fromFav.attractions,
            ]),
            restaurants: dedupById([
              ...toFav.restaurants,
              ...fromFav.restaurants,
            ]),
          };
          useFavoritesStore.setState((s) => ({
            profiles: { ...s.profiles, [id]: mergedFav },
          }));

          // Route planner: prefer existing user state, else adopt guest
          const route = useRoutePlannerStore.getState();
          const fromRoute: RoutePlannerState =
            route.profiles[guestId] || route.profiles.global || route.state;
          const toRoute: RoutePlannerState = route.profiles[id] || route.state;
          const isEmpty = (rp: RoutePlannerState) =>
            rp.itinerary.length === 0 &&
            rp.currentStep === 0 &&
            !rp.selectedScheme;
          const mergedRoute = isEmpty(toRoute) ? fromRoute : toRoute;
          useRoutePlannerStore.setState((s) => ({
            profiles: { ...s.profiles, [id]: mergedRoute },
            state: mergedRoute,
          }));
  ...
  }, [status, session, setUserFromOAuth, logout, user]);
